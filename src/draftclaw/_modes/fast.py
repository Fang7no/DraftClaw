...
from draftclaw._core.contracts import DocumentText, ErrorItem, ErrorMergeOutput, LLMRoundOutput, ModeResult, ModeStats
...
from draftclaw._postprocess.reconcile import filter_confirmed_checklist, reconcile_round
...
class FastMode:
    ...
    async def run(self, *, document: DocumentText, history: HistoryStore) -> ModeResult:
        ...
        self._report_agent_progress(current=0, total=total_chunks)
        checklist = []
        ...
        for chunk_no, chunk_text in enumerate(chunks, start=1):
            ...
            low_level_messages, low_level_rendered = self.prompt_builder.build_messages(
                ...
                user_file="modes/fast_low_level_review.md",
                variables={
                    "chunk_no": chunk_no,
            ...
            history.save_prompt("fast_main", chunk_no, main_rendered)
            history.save_prompt("fast_low_level", chunk_no, low_level_rendered)
            ...
            history.save_raw_response("fast_main", chunk_no, raw_main)
            history.save_parsed_response("fast_main", chunk_no, parsed_main.model_dump(mode="json"))
            history.save_raw_response("fast_low_level", chunk_no, raw_low_level)
            history.save_parsed_response("fast_low_level", chunk_no, parsed_low_level.model_dump(mode="json"))

            main_error_batches.append((f"fast_round_{chunk_no}_main_context", list(parsed_main.errorlist)))
            low_level_error_batches.append((f"fast_round_{chunk_no}_local_chunk", list(parsed_low_level.errorlist)))

            combined_round = LLMRoundOutput(
                checklist=parsed_main.checklist,
            ...
            checklist = reconciled.checklist
            state_errorlist = renumber_error_items(reconciled.errorlist)
            history.save_state(chunk_no, checklist, state_errorlist)
            self._report_agent_progress(current=chunk_no, total=total_chunks)
        ...
        if merge_batches:
            ...
            history.save_raw_response("fast_merge", 1, "PARALLEL_CHANNEL_LOCAL_COMBINE")
        ...
        remaining_checklist = filter_confirmed_checklist(checklist, final_errorlist)
        history.save_state("merge", remaining_checklist, final_errorlist)

        mode_elapsed_ms = max(1, int((perf_counter() - mode_start) * 1000))
        history.save_timing(stage="fast_total", step="all", elapsed_ms=mode_elapsed_ms)

        return ModeResult(
            mode=ModeName.FAST,
            checklist=remaining_checklist,
            ...
            final_summary=(
                f"Fast mode finished with {len(final_errorlist)} confirmed errors "
                f"and {len(remaining_checklist)} pending checks."
            ),
    ...
    @staticmethod
    def _compact_checklist_json(items) -> str:  # noqa: ANN001
        compact = [
            {
